 ...
 *
 * @see <a href="https://minecraft.wiki/w/Region_file_format">Region file
 *      format</a>
 * @see <a href=
 *      "https://github.com/Minestom/Hephaistos/blob/master/common/src/main/kotlin/org/jglrxavpok/hephaistos/mca/RegionFile.kt">Hephaistos
 *      implementation</a>
 */
final class RegionFile implements AutoCloseable {
  ...
  private static final int CHUNK_HEADER_LENGTH = 4 + 1; // Length + Compression type (todo non constant to support
                                                        // custom compression)
  ...
  public void writeChunkData(int chunkX, int chunkZ, @NotNull CompoundBinaryTag data) throws IOException {
    ...
    lock.lock();
    try {
      // We don't attempt to reuse the current allocation, just write it to a new
      // position and free the old one.
  ...
  private void readHeader() throws IOException {
    ...
    final long totalSectors = ((file.length() - 1) / SECTOR_SIZE) + 1; // Round up, last sector does not need to be full
                                                                       // size
